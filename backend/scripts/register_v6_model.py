"""
Register v6.0 Kaggle-adapted model in the database
"""
...
async def register_model():
    model_path = Path(__file__).parent.parent / "models" / "v6.0_model.pkl"
    metadata_path = Path(__file__).parent.parent / "models" / "v6.0_metadata.json"
    ...
    async with async_session() as session:
        ...
        print(f"\n✅ SUCCESS! Registered model v6.0")
        print(f"  Path: {model_path}")
        print(f"  SHA256: {sha256[:16]}...")
        print(f"\n📊 Performance Metrics (Real Kaggle Data):")
        print(f"  Data Source:  Kaggle Credit Card Fraud Dataset")
        print(f"  Total Transactions: {metadata['metrics']['dataset_rows']:,}")
        print(f"  Fraud Rate: {metadata['metrics']['dataset_fraud_pct']:.4f}%")
        print(f"")
        print(f"  Recall:     {metadata['metrics']['recall']*100:.2f}%  ✓")
        print(f"  Precision:  {metadata['metrics']['precision']*100:.2f}%")
        print(f"  FPR:        {metadata['metrics']['fpr']*100:.4f}%  ✓")
        print(f"  F1-Score:   {metadata['metrics']['f1_score']:.4f}")
        print(f"  AUC-ROC:    {metadata['metrics']['auc_roc']:.4f}")
        print(f"  Specificity: {metadata['metrics']['specificity']*100:.2f}%")
        print(f"\n🎯 Model is now ACTIVE and will be loaded on next restart")
        print(f"\n💡 This model is trained on REAL credit card fraud data!")
